...
function hsts_pro_render_tab_panels(): void {
    ...
    <div class="hsts-pro-panel" data-tab="faq" style="display:none;padding-top:24px;">
        ...
        $faqs = array(
            ...
            array('general',__('Does it work with Nginx, LiteSpeed, or IIS?','headers-security-advanced-hsts-wp'),__('Yes. The plugin sends every header a single time via PHP, on any server. It does not write to .htaccess, so there is one source and no duplicate headers.','headers-security-advanced-hsts-wp')),
            array('general',__('Does it work with caching plugins?','headers-security-advanced-hsts-wp'),__('Yes, for pages served through PHP. One honest caveat: some page caches (e.g. WP Super Cache in Expert/mod_rewrite mode, or Cache Enabler) serve cached pages as static HTML directly from the web server, bypassing PHP; those responses do not receive the plugin headers, because as of 5.3.4 headers are emitted only via PHP. PHP-served caching (WP Super Cache Simple mode, W3 Total Cache, LiteSpeed Cache, WP Rocket) is unaffected. Coverage for static/rewrite-served responses is planned as an opt-in feature.','headers-security-advanced-hsts-wp')),
            ...
            array('general',__('Can it conflict with other security plugins?','headers-security-advanced-hsts-wp'),__('Rarely. If another plugin or your server already sets the same header, tick the matching box under "Disable individual headers" in Settings to stop this plugin emitting that one.','headers-security-advanced-hsts-wp')),
            ...
            array('troubleshooting',__('I see "Duplicate header" warnings','headers-security-advanced-hsts-wp'),__('Since version 5.3.4 the plugin sends each header only once via PHP, so it cannot duplicate its own headers. If a header is still doubled, your server or another plugin is also sending it: go to Settings → "Disable individual headers" and tick that header to let the other source win.','headers-security-advanced-hsts-wp')),
